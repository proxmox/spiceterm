...
static uint8_t my_kbd_get_leds(SpiceKbdInstance *sin) { return 0; }
...
static const name2keysym_t *lookup_keysym(const char *name) {
...
static int agent_owns_clipboard[256] = {
    0,
};

static void vdagent_reply(spiceTerm *vt, uint32_t type, uint32_t error) {
    uint32_t size;
...
static void dump_message(unsigned char *buf, int size) {
    int i;
...
static void vdagent_send_clipboard(spiceTerm *vt, uint8_t selection) {
    ...
    if (vt->utf8) {
        ...
        int i;
        for (i = 0; i < sel_len; i++) {
            sel_data[i] = (char)vt->selection[i];
        }
        sel_data[sel_len] = 0;
    }
...
static int vmc_write(SpiceCharDeviceInstance *sin, const uint8_t *buf, int len) {
    ...
    case VD_AGENT_MONITORS_CONFIG: {
        ...
        DPRINTF(
            1, "VD_AGENT_MONITORS_CONFIG %d %d %d", list->num_of_monitors, list->monitors[0].width,
            list->monitors[0].height
        );
...
static int vmc_read(SpiceCharDeviceInstance *sin, uint8_t *buf, int len) {
    ...
    if (size < vdagent_write_buffer_pos) {
        memmove(vdagent_write_buffer, vdagent_write_buffer + size, vdagent_write_buffer_pos - size);
    }
...
static void vmc_state(SpiceCharDeviceInstance *sin, int connected) { /* IGNORE */ }
...
static void add_keymap_entry(guint8 mask, guint8 keycode, guint keysym, guint unicode) {
...
static gboolean parse_keymap(const char *language) {
    char line[1024];
    int len;
    ...
    for (;;) {
        ...
        if (!strncmp(line, "include ", 8)) {
            ...
            if (tok[0] == 'd' && tok[1] == 'e' && tok[2] == 'a' && tok[3] == 'd' && tok[4] == '_') {
                continue;
            }
            ...
            if (addupper) {
                ...
                if (uc != line[0]) {
                    ...
                    if ((map = lookup_keysym(ucname))) {
                        add_keymap_entry(mask | MOD_MASK_SHIFT, keycode, map->keysym, map->unicode);
                    }
                }
            }
        }
