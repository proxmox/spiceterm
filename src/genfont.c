...
static int font_add_glyph(const char *data) {
...
static int load_psf_font(const char *filename, int is_default) {
    ...
    if (!PSF_MAGIC_OK(psfhdr) || !PSF_MODE_VALID(psfhdr.mode) || !PSF_MODE_HASSFM(psfhdr.mode) ||
        (psfhdr.charheight != 16)) {
    ...
    for (glyph = 0; glyph < charcount; glyph++) {
        int fi = 0;
        while (gzread(f, &unichar, sizeof(unicode)) == sizeof(unicode) && (unichar != PSF_SEPARATOR)
        ) {
